...
import math
from model import Causal_GCN, GraphConvolution
...
def configure_optimizers(model, learning_rate, weight_decay):
    ...
    no_decay = set()
    weight_decay = 1e-4
    whitelist_weight_modules = (torch.nn.Linear, torch.nn.Conv1d, Causal_GCN, GraphConvolution, torch.nn.Conv2d)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            elif pn.endswith('weight1') and isinstance(m, whitelist_weight_modules):
                # weights of whitelist modules will be weight decayed
                decay.add(fpn)
            elif pn.endswith('weight1') and isinstance(m, blacklist_weight_modules):
                # weights of blacklist modules will NOT be weight decayed
                no_decay.add(fpn)
